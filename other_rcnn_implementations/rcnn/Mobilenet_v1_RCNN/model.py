...
from Mobilenet_v1_RCNN import utils
...
def _conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1), train_bn=True):
    ...
    return relu6(name='conv1_relu')(x)
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, alpha,
    ...
    x = relu6(name='conv_dw_%d_relu' % block_id)(x)
    ...
    return relu6(name='conv_pw_%d_relu' % block_id)(x)


def mobilenet_graph(img_input, architecture, ABLATION_FLAGS, alpha=1.0, depth_multiplier=1, train_bn=True):
    assert architecture in ["mobilenet224v1"]
    mobilenet_depths = ABLATION_FLAGS['mobilenet_depths']
    # Stage 1
    x = _conv_block(img_input, mobilenet_depths[0], alpha, strides=(2, 2), train_bn=train_bn)
    C1 = x = _depthwise_conv_block(x, mobilenet_depths[0], alpha, 
                                   depth_multiplier, block_id=1, train_bn=train_bn)

    # Stage 2
    x = _depthwise_conv_block(x, mobilenet_depths[1], alpha, depth_multiplier,
                              strides=(2, 2), block_id=2, train_bn=train_bn)
    C2 = x = _depthwise_conv_block(x, mobilenet_depths[1], alpha, 
                                   depth_multiplier, block_id=3, train_bn=train_bn)

    # Stage 3
    x = _depthwise_conv_block(x, mobilenet_depths[2], alpha, depth_multiplier,
                              strides=(2, 2), block_id=4, train_bn=train_bn)
    C3 = x = _depthwise_conv_block(x, mobilenet_depths[2], alpha, 
                                   depth_multiplier, block_id=5, train_bn=train_bn)

    # Stage 4
    x = _depthwise_conv_block(x, mobilenet_depths[3], alpha, depth_multiplier,
                            strides=(2, 2), block_id=6, train_bn=train_bn)
    x = _depthwise_conv_block(x, mobilenet_depths[3], alpha, 
    ...
    C4 = x = _depthwise_conv_block(x, mobilenet_depths[3], alpha, 
                                   depth_multiplier, block_id=11, train_bn=train_bn)

    # Stage 5
    x = _depthwise_conv_block(x, mobilenet_depths[4], alpha, depth_multiplier,
                            strides=(2, 2), block_id=12, train_bn=train_bn)
    C5 = x = _depthwise_conv_block(x, mobilenet_depths[4], alpha, 
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride, ABLATION_FLAGS):
    ...
    rpn_graph_depth = ABLATION_FLAGS['rpn_graph_depth']
    shared = KL.Conv2D(rpn_graph_depth, (3, 3), padding='same', activation='relu',
...
def build_rpn_model(anchor_stride, anchors_per_location, depth, ABLATION_FLAGS):
    ...
    outputs = rpn_graph(input_feature_map, anchors_per_location, anchor_stride, ABLATION_FLAGS)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, ABLATION_FLAGS, train_bn=True):
    ...
    Returns:
        ...
        bbox_deltas: [N, (dy, dx, log(dh), log(dw))] Deltas to apply to
                     proposal boxes
    """
    FPN_depths = ABLATION_FLAGS['FPN_depths']
    # ROI Pooling
    # Shape: [batch, num_boxes, pool_height, pool_width, channels]
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_classifier")([rois, image_meta] + feature_maps)
    # Two 1024 FC layers (implemented with Conv2D for consistency)
    x = KL.TimeDistributed(KL.Conv2D(FPN_depths[0], (pool_size, pool_size), padding="valid"),
                           name="mrcnn_class_conv1")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_class_bn1')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Conv2D(FPN_depths[1], (1, 1)),
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    # Conv layers
    fpn_n_blocks = ABLATION_FLAGS['fpn_n_blocks']
    fpn_mask_depth = ABLATION_FLAGS['fpn_mask_depth']

    for i in range(1, fpn_n_blocks + 1):
        x = KL.TimeDistributed(KL.Conv2D(fpn_mask_depth, (3, 3), padding="same"),
                            name=f'mrcnn_mask_conv{i}')(x)
        x = KL.TimeDistributed(BatchNorm(),
                            name=f'mrcnn_mask_bn{i}')(x, training=train_bn)
        x = KL.Activation('relu')(x)


    x = KL.TimeDistributed(KL.Conv2DTranspose(fpn_mask_depth, (2, 2), strides=2, activation="relu"),
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, ABLATION_FLAGS = {}):
        """
        ...
        self.ABLATION_FLAGS = {
            'mobilenet_depths': (32, 64, 64, 128, 128),
            'rpn_graph_depth': 256,
            'FPN_depths': (64, 64),
            'fpn_n_blocks': 1,
            'fpn_mask_depth': 64,
            'mrcnn_depth_backbone': 64,
        }
        self.ABLATION_FLAGS.update(ABLATION_FLAGS)
        if mode == 'training':
            os.mkdir(self.log_dir)
            json.dump(self.ABLATION_FLAGS, open(os.path.join(self.log_dir, 'ABLATION_FLAGS.json'), 'w'))
    ...
    def build(self, mode, config):
        ...
        if config.BACKBONE in ["mobilenet224v1"]:
            _, C2, C3, C4, C5 = mobilenet_graph(input_image, config.BACKBONE, self.ABLATION_FLAGS,
        ...
        mrcnn_depth_backbone = self.ABLATION_FLAGS['mrcnn_depth_backbone'] 
        depths = [mrcnn_depth_backbone] * 4 

        P5 = KL.Conv2D(depths[0], (1, 1), name='fpn_c5p5')(C5)
        P4 = KL.Add(name="fpn_p4add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
            KL.Conv2D(depths[1], (1, 1), name='fpn_c4p4')(C4)])
        P3 = KL.Add(name="fpn_p3add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P4),
            KL.Conv2D(depths[2], (1, 1), name='fpn_c3p3')(C3)])
        P2 = KL.Add(name="fpn_p2add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
            KL.Conv2D(depths[3], (1, 1), name='fpn_c2p2')(C2)])
        # Attach 3x3 conv to all P layers to get the final feature maps.
        P2 = KL.Conv2D(depths[3], (3, 3), padding="SAME", name="fpn_p2")(P2)
        P3 = KL.Conv2D(depths[2], (3, 3), padding="SAME", name="fpn_p3")(P3)
        P4 = KL.Conv2D(depths[1], (3, 3), padding="SAME", name="fpn_p4")(P4)
        P5 = KL.Conv2D(depths[0], (3, 3), padding="SAME", name="fpn_p5")(P5)
        ...
        rpn = build_rpn_model(config.RPN_ANCHOR_STRIDE,
                              len(config.RPN_ANCHOR_RATIOS), mrcnn_depth_backbone, self.ABLATION_FLAGS) # 
